	...
	public static void main(String[] args) {
		...
		for (int col = 0; col < grid[0].length; col++) {
	        for (int row = 0; row < grid.length; row++) {
	            ...
	            // horizontal
	            if (col < grid[0].length - seriesLen) {
	                tempProd = grid[col][row];
	                for (int i = 1; i < seriesLen; i++) {
	            ...
	            // diagonal NE -> SW
	            if ((col < grid.length - seriesLen) && (row >= seriesLen)) {
	                tempProd = grid[col][row];
	                for (int i = 1; i < seriesLen; i++) {
	            ...
	            // diagonal NW -> SE
	            if ((row < grid.length - seriesLen) && (col < grid[0].length - seriesLen)) {
	                tempProd = grid[col][row];
	                for (int i = 1; i < seriesLen; i++) {
	                    tempProd *= grid[col + i][row + i];
	                }
